...
\begin{description}
\item[Note]--- Whenever the term ``count'' or ``parasite count'' is used in this dissertation, in the text or in plot axes, it should always be interpreted as a derived count per \micro\liter\; it is never an actual raw count.
\end{description}
...
\caption{Pre-dose parasite counts per \micro\liter\ by sex, centre and treatment, with box plots showing median and quartiles.}
\label{preaov}
\end{center}
\end{figure}
...
\begin{itemize}
\item The parasite data appears to be right-skewed and a logarithmic transformation seems to be appropriate to remedy this. Wootton \textit{et al.} chose a logarithmic transformation of the parasite count in their study of similar data \cite{wootton}.
